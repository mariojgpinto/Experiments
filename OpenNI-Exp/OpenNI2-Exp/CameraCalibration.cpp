...
int main_kinect_calibration(int argc, char* argv[]){
    ...
    while (success < numBoards)
    {
		...
		if(!kinect->get_ir(img2) || !img2.rows)
			break;
    ...
    {    
		...
		if(!kinect->get_color(img2) || !img2.rows)
			break;

		if(!kinect->get_depth_8(depth) || !depth.rows)
			break;

        remap(img1, imgU1, map1x, map1y, INTER_LINEAR, BORDER_CONSTANT, Scalar());
        remap(img2, imgU2, map2x, map2y, INTER_LINEAR, BORDER_CONSTANT, Scalar());
		remap(depth, imgU3, map2x, map2y, INTER_LINEAR, BORDER_CONSTANT, Scalar());
		//remap(depth, imgU3, map1x, map1y, INTER_LINEAR, BORDER_CONSTANT, Scalar());

		imgU1.copyTo(temp1, imgU3);
		imgU2.copyTo(temp2, imgU3);
