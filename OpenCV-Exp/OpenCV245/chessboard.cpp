...
int main_chessboard( int argc, char* argv[] ){
	...
	while( cv::waitKey(31) !=27 ){
		...
		if(found){
			cornerSubPix( viewGray, pointbuf, Size(11,11), cv::Size(-1,-1), TermCriteria( CV_TERMCRIT_EPS+CV_TERMCRIT_ITER, 30, 0.01 ));
			drawChessboardCorners( view, boardSize, Mat(pointbuf), found );
			cv::circle(view,cv::Point(pointbuf[0].x,pointbuf[0].y),9,cv::Scalar(0,0,255),-1);
			cv::circle(view,cv::Point(pointbuf[boardSize.width-1].x,pointbuf[boardSize.width-1].y),9,cv::Scalar(0,0,255),-1);
			cv::circle(view,cv::Point(pointbuf[boardSize.width*(boardSize.height-1)].x,pointbuf[boardSize.width*(boardSize.height-1)].y),9,cv::Scalar(0,0,255),-1);
			cv::circle(view,cv::Point(pointbuf[boardSize.width*boardSize.height -1].x,pointbuf[boardSize.width*boardSize.height -1].y),9,cv::Scalar(0,0,255),-1);
		}
