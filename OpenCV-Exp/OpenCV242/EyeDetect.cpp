...
cv::CascadeClassifier eye_cascade2;
...
int detectEye2(cv::Mat& im, cv::Mat& tpl, cv::Rect& rect)
{
	std::vector<cv::Rect> faces, eyes;
	face_cascade.detectMultiScale(im, faces, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, cv::Size(30,30));
	...
	{
		cv::Mat face = im(faces[i]);
		eye_cascade2.detectMultiScale(face, eyes, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, cv::Size(20,20));
...
int main_eye_detect(int argc, char** argv)
{
	...
	eye_cascade2.load("haarcascades\\haarcascade_mcs_righteye.xml");
	...
	while (cv::waitKey(15) != 'q')
	{
		...
		if (eye_bb2.width == 0 && eye_bb2.height == 0)
		{
			// Detection stage
			// Try to detect the face and the eye of the user
			detectEye2(gray, eye_tpl2, eye_bb2);
		}
		else
		{
			// Tracking stage with template matching
			trackEye(gray, eye_tpl2, eye_bb2);

			// Draw bounding rectangle for the eye
			cv::rectangle(frame, eye_bb2, CV_RGB(255,0,0));
		}
