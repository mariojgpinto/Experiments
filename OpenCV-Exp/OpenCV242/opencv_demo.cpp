...
void colorizeDisparity( const Mat& gray, Mat& rgb, double maxDisp=-1.f, float S=1.f, float V=1.f )
{
	...
	unsigned int dist = ((uchar)maxDisp - 60) * 240 / (uchar)maxDisp;
 ...
 */
int main_opencv_demo( int argc, char* argv[] )
{
	...
	cvNamedWindow("colorized disparity map");
	setMouseCallback( "colorized disparity map", onMouse, 0 );
...
Mat show_;
        ...
        else
        {
            ...
            depthMap.convertTo( show_, CV_8UC1, scaleFactor );
            imshow( "depth map", show_ );
			if(c == 's')
				imwrite("depth.bmp",show_);
            ...
            if( retrievedImageFlags[1] && capture.retrieve( disparityMap, CV_CAP_OPENNI_DISPARITY_MAP ) )
            {
                if( isColorizeDisp )
                {
                    ...
                    colorizeDisparity( disparityMap, colorDisparityMap, isFixedMaxDisp ? getMaxDisparity(capture) : 50 );
                    Mat validColorDisparityMap;
                    colorDisparityMap.copyTo( validColorDisparityMap, disparityMap != 0 );
					
					circle(validColorDisparityMap,Point(xx,yy),3,Scalar(255,255,255),3);
					putText(validColorDisparityMap,format("distance: %f m",dist),Point(5,15),FONT_HERSHEY_PLAIN,1,Scalar(255));
            ...
            if( retrievedImageFlags[3] && capture.retrieve( bgrImage, CV_CAP_OPENNI_BGR_IMAGE ) ){
				bgrImage.copyTo(bgrImage2,validDepthMap);

				imshow( "rgb image", bgrImage2 );
			}
